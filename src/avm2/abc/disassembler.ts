...
module Shumway.AVM2.ABC {
  ...
  function traceArray(writer, name, array, abc = null) {
    if (array.length === 0) {
      return;
    }
    writer.enter(name + " {");
  ...
  function traceMultinamesOnly(constantPool: ConstantPool, writer) {
  ...
  function traceOperands(opcode, abc, code, rewind = false) {
  ...
  var SourceTracer = (function () {
    ...
    function getSignature(mi, excludeTypesAndDefaultValues = false) {
    ...
    SourceTracer.prototype = {
      traceTraits: function traceTraits(traits, isStatic, inInterfaceNamespace) {
        ...
        traits.forEach(function (trait) {
          ...
          if (trait.isSlot() || trait.isConst()) {
            ...
            if (trait.attributes & ATTR.Override) {
              str += " override";
            }
            if (mi.isNative()) {
              str += " native";
      ...
      traceClassStub2: function traceClassStub(trait) {
        ...
        function traceTraits(traits, isStatic = false) {
      ...
      traceClassStub: function traceClassStub(trait) {
        ...
        function writeTraits(traits, isNative, isStatic = false) {
      ...
      s.traits.forEach(function (t) {
        if (t.isClass()) {
          ...
          t.classInfo.instanceInfo.traits.forEach(function (it) {
            if (it.isMethod() && !(it.attributes & ATTR.Override)) {
